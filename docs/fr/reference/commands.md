...
Le nom du répertoire détermine le nom du skill dans votre IDE. Par exemple, le répertoire `rmbaid-analyst/` enregistre le skill `rmbaid-analyst`.
...
Exécutez `rmbaid-help` pour obtenir des conseils contextuels sur votre prochaine étape.
...
| --- | --- | --- |
| `rmbaid-analyst` | Mary (Analyste) | Brainstorming de projets, recherche, création de briefs |
| `rmbaid-architect` | Winston (Architecte) | Conçoit l'architecture système |
| `rmbaid-ux-designer` | Sally (Designer UX) | Crée les designs UX |
| `rmbaid-tech-writer` | Paige (Rédacteur Technique) | Documente les projets, rédige des guides, génère des diagrammes |
...
| --- | --- |
| `rmbaid-create-prd` | Créer un PRD[^1] |
| `rmbaid-create-architecture` | Concevoir l'architecture système |
| `rmbaid-create-epics-and-stories` | Créer des epics et des stories |
| `rmbaid-dev-story` | Implémenter une story |
| `rmbaid-code-review` | Effectuer une revue de code |
| `rmbaid-quick-dev` | Flux rapide unifié — clarifier l'intention, planifier, implémenter, réviser, présenter |
...
**RMB_AID-Help : Votre guide intelligent**

`rmbaid-help` est votre interface principale pour découvrir quoi faire ensuite. Il inspecte votre projet, comprend les requêtes en langage naturel et recommande la prochaine étape requise ou optionnelle en fonction de vos modules installés.
...
Tous les skills utilisent le préfixe `rmbaid-` suivi d'un nom descriptif (ex. `rmbaid-analyst`, `rmbaid-create-prd`, `rmbaid-help`). Consultez [Modules](./modules.md) pour les modules disponibles.
...
**Des skills attendus sont manquants.** L'installateur génère uniquement les skills pour les modules que vous avez sélectionnés. Exécutez à nouveau `npx rmb-aid install` et vérifiez votre sélection de modules. Vérifiez que les fichiers de skill existent dans le répertoire attendu.
...
[^2]: Persona : dans le contexte de RMB_AID, une persona désigne un agent IA avec un rôle défini, un style de communication et une expertise spécifiques (ex. Mary l'analyste, Winston l'architecte). Chaque persona garde son "caractère" pendant les interactions.
