...
- `rmbaid-create-architecture` — respecte les préférences techniques pendant la phase de solutioning
- `rmbaid-create-story` — informe la création de stories avec les patterns du projet
- `rmbaid-dev-story` — guide les décisions d'implémentation
- `rmbaid-code-review` — valide par rapport aux standards du projet
- `rmbaid-quick-dev` — applique les patterns lors de l'implémentation des spécifications techniques
- `rmbaid-sprint-planning`, `rmbaid-retrospective`, `rmbaid-correct-course` — fournit le contexte global du projet
...
|------------------------------------------|-----------------------------------------------------|---------------------------------------------------------------------------------------|
| **Nouveau projet, avant l'architecture** | Manuellement, avant `rmbaid-create-architecture`      | Documenter vos préférences techniques pour que l'architecte les respecte              |
| **Nouveau projet, après l'architecture** | Via `rmbaid-generate-project-context` ou manuellement | Capturer les décisions d'architecture pour les agents d'implémentation                |
| **Projet existant**                      | Via `rmbaid-generate-project-context`                 | Découvrir les patterns existants pour que les agents suivent les conventions établies |
| **Projet Quick Dev**                   | Avant ou pendant `rmbaid-quick-dev`                   | Garantir que l'implémentation rapide respecte vos patterns                            |
...
Créez le fichier `_rmbaid-output/project-context.md` et ajoutez vos règles :

```bash
# Depuis la racine du projet
mkdir -p _rmbaid-output
touch _rmbaid-output/project-context.md
```
...
Exécutez le workflow `rmbaid-generate-project-context` après avoir terminé votre architecture :
...
Pour les projets existants, exécutez `rmbaid-generate-project-context` pour découvrir les patterns existants :
...
Vous pouvez l'éditer manuellement à tout moment, ou réexécuter `rmbaid-generate-project-context` pour le mettre à jour après des changements significatifs.

:::note[Emplacement du Fichier]
L'emplacement par défaut est `_rmbaid-output/project-context.md`. Les workflows le recherchent là, et vérifient également `**/project-context.md` n'importe où dans votre projet.
:::
