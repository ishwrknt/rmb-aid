 ...
 *
 * Validates cross-file references in RMB_AID source files (agents, workflows, tasks, steps).
 * Catches broken file paths, missing referenced files, and absolute path leaks.
 *
 * What it checks:
 * - {project-root}/_rmbaid/ references in YAML and markdown resolve to real src/ files
...
// Pattern: {project-root}/_rmbaid/ references
const PROJECT_ROOT_REF = /\{project-root\}\/_rmbaid\/([^\s'"<>})\]`]+)/g;

// Pattern: {_rmbaid}/ shorthand references
const RMB_AID_SHORTHAND_REF = /\{_rmbaid\}\/([^\s'"<>})\]`]+)/g;
...
function mapInstalledToSource(refPath) {
  // Strip {project-root}/_rmbaid/ or {_rmbaid}/ prefix
  let cleaned = refPath.replace(/^\{project-root\}\/_rmbaid\//, '').replace(/^\{_rmbaid\}\//, '');

  // Also handle bare _rmbaid/ prefix (seen in some invoke-task)
  cleaned = cleaned.replace(/^_rmbaid\//, '');

  // Skip install-only paths (generated at install time, not in source)
  if (isInstallOnly(cleaned)) return null;

  // core/, rmbaid/, and utility/ are directly under src/
  if (cleaned.startsWith('core/') || cleaned.startsWith('rmbaid/') || cleaned.startsWith('utility/')) {
...
function isInstallOnly(cleanedPath) {
  // Skip paths that only exist in the installed _rmbaid/ structure, not in src/
...
function extractYamlRefs(filePath, content) {
  ...
  function checkValue(value, range, keyPath) {
    ...
    // Check for {project-root}/_rmbaid/ refs
    const prMatch = value.match(/\{project-root\}\/_rmbaid\/[^\s'"<>})\]`]+/);
    if (prMatch) {
      refs.push({ file: filePath, raw: prMatch[0], type: 'project-root', line, key: keyPath });
    }

    // Check for {_rmbaid}/ refs
    const bmMatch = value.match(/\{_rmbaid\}\/[^\s'"<>})\]`]+/);
    if (bmMatch) {
...
function extractMarkdownRefs(filePath, content) {
  ...
  // {project-root}/_rmbaid/ refs
  runPattern(PROJECT_ROOT_REF, 'project-root');

  // {_rmbaid}/ shorthand
  runPattern(RMB_AID_SHORTHAND_REF, 'project-root');
...
function resolveRef(ref) {
  ...
  if (ref.type === 'exec-attr') {
    ...
    if (execPath.includes('{_rmbaid}')) {
      return mapInstalledToSource(execPath);
    }
    if (execPath.startsWith('_rmbaid/')) {
      return mapInstalledToSource(execPath);
    }
    // Relative exec path
  ...
  if (ref.type === 'invoke-task') {
    ...
    const prMatch = ref.raw.match(/\{project-root\}\/_rmbaid\/([^\s'"<>})\]`]+)/);
    if (prMatch) return mapInstalledToSource(prMatch[0]);

    const bmMatch = ref.raw.match(/\{_rmbaid\}\/([^\s'"<>})\]`]+)/);
    if (bmMatch) return mapInstalledToSource(bmMatch[0]);

    const bareMatch = ref.raw.match(/_rmbaid\/([^\s'"<>})\]`]+)/);
