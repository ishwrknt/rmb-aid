...
class UI {
  ...
  async promptInstall(options = {}) {
    ...
    const { rmbaidDir } = await installer.findRmbaidDir(confirmedDirectory);

    // Check if there's an existing RMB_AID installation
    const hasExistingInstall = await fs.pathExists(rmbaidDir);
    ...
    if (hasExistingInstall) {
      ...
      const { existingInstall, rmbaidDir } = await this.getExistingInstallation(confirmedDirectory);
      ...
      // Common actions
      choices.push({ name: 'Modify RMB_AID Installation', value: 'update' });
      ...
      if (actionType === 'update') {
        ...
        if (options.customContent) {
          ...
          const cacheDir = path.join(rmbaidDir, '_config', 'custom');
  ...
  async promptToolSelection(projectDir, options = {}) {
    ...
    const { Installer } = require('./core/installer');
    const installer = new Installer();
    const { rmbaidDir } = await installer.findRmbaidDir(projectDir || process.cwd());
   ...
   * @returns {Object} Object with existingInstall, installedModuleIds, and rmbaidDir
   */
  async getExistingInstallation(directory) {
    const { ExistingInstall } = require('./core/existing-install');
    const { Installer } = require('./core/installer');
    const installer = new Installer();
    const { rmbaidDir } = await installer.findRmbaidDir(directory);
    const existingInstall = await ExistingInstall.detect(rmbaidDir);
    const installedModuleIds = new Set(existingInstall.moduleIds);

    return { existingInstall, installedModuleIds, rmbaidDir };
  }
  ...
  async selectAllModules(installedModuleIds = new Set()) {
    ...
    allOptions.push({ label: 'RMB_AID Core Module', value: 'core', hint: 'Core configuration and shared resources' });
    ...
    // Group 2: RMB_AID Official Modules (type: rmbaid-org)
    const officialModules = [];
    for (const mod of externalModules) {
      if (mod.type === 'rmbaid-org') {
  ...
  async getDefaultModules(installedModuleIds = new Set()) {
    ...
    // Add default-selected local modules (typically RMB_AID)
    for (const mod of localModules) {
    ...
    // If no defaults found, use 'rmbaid' as the fallback default
    if (defaultModules.length === 0) {
      defaultModules.push('rmbaid');
    }
  ...
  async displayDirectoryInfo(directory) {
    ...
    if (dirExists) {
      ...
      if (stats.isDirectory()) {
        ...
        if (files.length > 0) {
          // Check for any rmbaid installation (any folder with _config/manifest.yaml)
          const { Installer } = require('./core/installer');
          const installer = new Installer();
          const rmbaidResult = await installer.findRmbaidDir(directory);
          const hasRmbaidInstall =
            (await fs.pathExists(rmbaidResult.rmbaidDir)) && (await fs.pathExists(path.join(rmbaidResult.rmbaidDir, '_config', 'manifest.yaml')));

          const rmbaidNote = hasRmbaidInstall ? ` including existing RMB_AID installation (${path.basename(rmbaidResult.rmbaidDir)})` : '';
          await prompts.log.message(`Directory exists and contains ${files.length} item(s)${rmbaidNote}`);
        } else {
  ...
  async displayStatus(statusData) {
    const { installation, modules, availableUpdates, rmbaidDir } = statusData;
    ...
    if (availableUpdates.length > 0) {
      ...
      await prompts.log.message('Run \'rmbaid install\' and select "Quick Update" to update');
    } else {
