  ...
  try {
    ...
    if (result && semver.gt(result, packageJson.version)) {
      ...
      const updateMsg = [
        ...
        `  npm cache clean --force && npx rmb-aid@${tag} install`,
      ].join('\n');
...
// Set up main program
program.version(packageJson.version).description('RMB_AID Core CLI - Universal AI agent framework');
