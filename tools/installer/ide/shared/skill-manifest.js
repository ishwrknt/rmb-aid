...
/**
 * Load rmbaid-skill-manifest.yaml from a directory.
 ...
 * @param {string} dirPath - Directory to check for rmbaid-skill-manifest.yaml
 * @returns {Object|null} Parsed manifest or null
 */
async function loadSkillManifest(dirPath) {
  const manifestPath = path.join(dirPath, 'rmbaid-skill-manifest.yaml');
  try {
    ...
    console.warn(`Warning: Failed to parse rmbaid-skill-manifest.yaml in ${dirPath}: ${error.message}`);
    return null;
  }
}
...
/**
 * Get the install_to_rmbaid flag for a specific file from a loaded skill manifest.
 * @param {Object|null} manifest - Loaded manifest (from loadSkillManifest)
 * @param {string} filename - Source filename to look up
 * @returns {boolean} install_to_rmbaid value (defaults to true)
 */
function getInstallToRmbaid(manifest, filename) {
  if (!manifest) return true;
  // Single-entry manifest applies to all files in the directory
  if (manifest.__single) return manifest.__single.install_to_rmbaid !== false;
  // Multi-entry: look up by filename directly
  if (manifest[filename]) return manifest[filename].install_to_rmbaid !== false;
  return true;
}

module.exports = { loadSkillManifest, getCanonicalId, getArtifactType, getInstallToRmbaid };
