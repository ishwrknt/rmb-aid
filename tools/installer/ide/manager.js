...
class IdeManager {
  constructor() {
    ...
    this.rmbaidFolderName = RMB_AID_FOLDER_NAME; // Default, can be overridden
  }

  /**
   * Set the rmbaid folder name for all IDE handlers
   * @param {string} rmbaidFolderName - The rmbaid folder name
   */
  setRmbaidFolderName(rmbaidFolderName) {
    this.rmbaidFolderName = rmbaidFolderName;
    // Update all loaded handlers
    for (const handler of this.handlers.values()) {
      if (typeof handler.setRmbaidFolderName === 'function') {
        handler.setRmbaidFolderName(rmbaidFolderName);
      }
    }
  }
  ...
  async loadConfigDrivenHandlers() {
    ...
    for (const [platformCode, platformInfo] of Object.entries(platformConfig.platforms)) {
      ...
      if (typeof handler.setRmbaidFolderName === 'function') {
        handler.setRmbaidFolderName(this.rmbaidFolderName);
      }
   ...
   */
  async setup(ideName, projectDir, rmbaidDir, options = {}) {
    ...
    try {
      const handlerResult = await handler.setup(projectDir, rmbaidDir, options);
