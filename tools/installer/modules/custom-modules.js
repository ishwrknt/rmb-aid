...
class CustomModules {
  ...
  async install(moduleName, rmbaidDir, fileTrackingCallback = null, options = {}) {
    ...
    const versionInfo = await manifest.getModuleVersionInfo(moduleName, rmbaidDir, sourcePath);
    await manifest.addModule(rmbaidDir, moduleName, {
   ...
   */
  async assembleQuickUpdateSources(config, existingInstall, rmbaidDir, externalModuleManager) {
    ...
    if (existingInstall.customModules) {
      for (const customModule of existingInstall.customModules) {
        ...
        if (sourcePath && sourcePath.startsWith('_config')) {
          // Paths are relative to RMB_AID dir, but we want absolute paths for install
          sourcePath = path.join(rmbaidDir, sourcePath);
