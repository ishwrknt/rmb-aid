...
const { RMB_AID_FOLDER_NAME } = require('../ide/shared/path-utils');
...
class Installer {
  ...
  async install(originalConfig) {
    ...
    try {
      ...
      const existingInstall = await ExistingInstall.detect(paths.rmbaidDir);
  ...
  async _validateIdeSelection(config) {
    ...
    if (suspendedIdes.length > 0 && suspendedIdes.length === config.ides.length) {
      ...
      throw new Error(
        `All selected tool(s) are suspended: ${suspendedIdes.join(', ')}. Installation aborted to prevent upgrading _rmbaid/ without a working IDE configuration.`,
      );
    }
  }
  ...
  async _cacheCustomModules(paths, addResult) {
    ...
    const customCache = new CustomModuleCache(paths.rmbaidDir);
  ...
  async _installAndConfigure(config, originalConfig, paths, officialModuleIds, allModules, addResult, officialModules) {
    ...
    installTasks.push({
      ...
      task: async (message) => {
        const verboseMode = process.env.RMB_AID_VERBOSE_INSTALL === 'true' || config.verbose;
        const moduleLogger = {
        ...
        if (config.modules && config.modules.length > 0) {
          for (const moduleName of config.modules) {
            ...
            const result = await officialModules.createModuleDirectories(moduleName, paths.rmbaidDir, {
    ...
    const configTask = {
      ...
      task: async (message) => {
        await this.generateModuleConfigs(paths.rmbaidDir, moduleConfigs);
        ...
        await manifestGen.generateManifests(paths.rmbaidDir, allModulesForManifest, [...this.installedFiles], {
          ides: config.ides || [],
          preservedModules: modulesForCsvPreserve,
        });

        message('Generating help catalog...');
        await this.mergeModuleHelpCatalogs(paths.rmbaidDir);
  ...
  async _setupIdes(config, allModules, paths, addResult) {
    ...
    for (const ide of validIdes) {
      const setupResult = await this.ideManager.setup(ide, paths.projectRoot, paths.rmbaidDir, {
        ...
        task: async (message) => {
          if (updateState.customFiles.length > 0) {
            ...
            for (const originalPath of updateState.customFiles) {
              const relativePath = path.relative(paths.rmbaidDir, originalPath);
          ...
          if (updateState.modifiedFiles.length > 0) {
            ...
            if (updateState.tempModifiedBackupDir && (await fs.pathExists(updateState.tempModifiedBackupDir))) {
              ...
              for (const modifiedFile of restoredModifiedFiles) {
                const relativePath = path.relative(paths.rmbaidDir, modifiedFile.path);
  ...
  async _prepareUpdateState(paths, config, existingInstall, officialModules) {
    ...
    const existingFilesManifest = await this.readFilesManifest(paths.rmbaidDir);
    const { customFiles, modifiedFiles } = await this.detectCustomFiles(paths.rmbaidDir, existingFilesManifest);
  ...
  async _backupUserFiles(paths, customFiles, modifiedFiles) {
    ...
    if (customFiles.length > 0) {
      tempBackupDir = path.join(paths.projectRoot, '_rmbaid-custom-backup-temp');
      await fs.ensureDir(tempBackupDir);

      for (const customFile of customFiles) {
        const relativePath = path.relative(paths.rmbaidDir, customFile);
    ...
    if (modifiedFiles.length > 0) {
      tempModifiedBackupDir = path.join(paths.projectRoot, '_rmbaid-modified-backup-temp');
  ...
  async _installCustomModules(config, paths, addResult, officialModules, ctx) {
    ...
    for (const [moduleName, sourcePath] of this.customModules.paths) {
      ...
      const result = await this.customModules.install(moduleName, paths.rmbaidDir, (filePath) => this.installedFiles.add(filePath), {
        moduleConfig: collectedModuleConfig,
      });

      // Generate runtime config.yaml with merged values
      await this.generateModuleConfigs(paths.rmbaidDir, {
   ...
   */
  async readFilesManifest(rmbaidDir) {
    const filesManifestPath = path.join(rmbaidDir, '_config', 'files-manifest.csv');
   ...
   */
  async detectCustomFiles(rmbaidDir, existingFilesManifest) {
    const customFiles = [];
    const modifiedFiles = [];

    // Memory is always in _rmbaid/_memory
    const rmbaidMemoryPath = '_memory';
    ...
    for (const fileEntry of existingFilesManifest) {
      if (fileEntry.path) {
        const absolutePath = path.join(rmbaidDir, fileEntry.path);
    ...
    // Recursively scan rmbaidDir for all files
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            const relativePath = path.relative(rmbaidDir, fullPath);
            ...
            if (relativePath.startsWith('_config/') || relativePath.startsWith('_config\\')) {
              ...
              if (relativePath.includes('/agents/') && fileName.endsWith('.customize.yaml')) {
                ...
                const manifestPath = path.join(rmbaidDir, '_config', 'manifest.yaml');
            ...
            if (relativePath.startsWith(rmbaidMemoryPath + '/') && path.dirname(relativePath).includes('-sidecar')) {
              continue;
            }
   ...
   */
  async generateModuleConfigs(rmbaidDir, moduleConfigs) {
    const yaml = require('yaml');
    ...
    const entries = await fs.readdir(rmbaidDir, { withFileTypes: true });
    ...
    for (const moduleName of installedModules) {
      const modulePath = path.join(rmbaidDir, moduleName);
...
# Generated by RMB_AID installer
...
`;
  ...
  /**
   * Merge all module-help.csv files into a single rmbaid-help.csv
   * Scans all installed modules for module-help.csv and merges them
   * Enriches agent info from agent-manifest.csv
   * Output is written to _rmbaid/_config/rmbaid-help.csv
   * @param {string} rmbaidDir - RMB_AID installation directory
   */
  async mergeModuleHelpCatalogs(rmbaidDir) {
    const allRows = [];
    const headerRow =
      'module,phase,name,code,sequence,workflow-file,command,required,agent-name,agent-command,agent-display-name,agent-title,options,description,output-location,outputs';

    // Load agent manifest for agent info lookup
    const agentManifestPath = path.join(rmbaidDir, '_config', 'agent-manifest.csv');
    ...
    if (await fs.pathExists(agentManifestPath)) {
      ...
      for (const line of lines) {
        ...
        if (cols.length >= 4) {
          ...
          // Build agent command: rmbaid:module:agent:name
          const agentCommand = module ? `rmbaid:${module}:agent:${agentName}` : `rmbaid:agent:${agentName}`;
    ...
    for (const [moduleName, modulePath] of modulePaths) {
      ...
      if (await fs.pathExists(helpFilePath)) {
        try {
          ...
          if (process.env.RMB_AID_VERBOSE_INSTALL === 'true') {
    ...
    const outputDir = path.join(rmbaidDir, '_config');
    await fs.ensureDir(outputDir);
    const outputPath = path.join(outputDir, 'rmbaid-help.csv');
    ...
    if (process.env.RMB_AID_VERBOSE_INSTALL === 'true') {
      await prompts.log.message(`  Generated rmbaid-help.csv: ${allRows.length} workflows`);
    }
  }
   ...
   * @param {Object} context - {rmbaidDir, modules, ides, customFiles, modifiedFiles}
   */
  async renderInstallSummary(results, context = {}) {
    ...
    if ((context.ides || []).length === 0) {
      lines.push(`  ${color.green('\u2713')}  No IDE selected ${color.dim('(installed in _rmbaid only)')}`);
    }

    // Context and warnings
    lines.push('');
    if (context.rmbaidDir) {
      lines.push(`  Installed to: ${color.dim(context.rmbaidDir)}`);
    }
    ...
    lines.push(
      '',
      '  Next steps:',
      `    Read our new Docs Site: ${color.dim('https://docs.rmb-aid.org/')}`,
      `    Join our Discord: ${color.dim('https://discord.gg/gk8jAdXWmj')}`,
      `    Star us on GitHub: ${color.dim('https://github.com/rmbaid-code-org/RMB_AID/')}`,
      `    Subscribe on YouTube: ${color.dim('https://www.youtube.com/@RMB_AIDCode')}`,
    );
    if (context.ides && context.ides.length > 0) {
      lines.push(`    Invoke the ${color.cyan('rmbaid-help')} skill in your IDE Agent to get started`);
    }

    await prompts.note(lines.join('\n'), 'RMB_AID is ready to use!');
  }
  ...
  async quickUpdate(config) {
    ...
    const { rmbaidDir } = await this.findRmbaidDir(projectDir);

    // Check if rmbaid directory exists
    if (!(await fs.pathExists(rmbaidDir))) {
      throw new Error(`RMB_AID not installed at ${rmbaidDir}. Use regular install for first-time setup.`);
    }

    // Detect existing installation
    const existingInstall = await ExistingInstall.detect(rmbaidDir);
    const installedModules = existingInstall.moduleIds;
    const configuredIdes = existingInstall.ides;
    const projectRoot = path.dirname(rmbaidDir);
  ...
  /**
   * Uninstall RMB_AID with selective removal options
   * @param {string} directory - Project directory
   * @param {Object} options - Uninstall options
   * @param {boolean} [options.removeModules=true] - Remove _rmbaid/ directory
  ...
  async uninstall(directory, options = {}) {
    ...
    const existingInstall = await ExistingInstall.detect(rmbaidDir);
    const outputFolder = await this._readOutputFolder(rmbaidDir);

    const removed = { modules: false, ideConfigs: false, outputFolder: false };

    // 2. IDE CLEANUP (before _rmbaid/ deletion so configs are accessible)
    ...
    // 4. RMB_AID DIRECTORY (last, after everything that needs it)
  ...
  async getStatus(directory) {
    const projectDir = path.resolve(directory);
    const { rmbaidDir } = await this.findRmbaidDir(projectDir);
    return await ExistingInstall.detect(rmbaidDir);
  }
   ...
   * Resolves rmbaidDir internally from projectDir
   * @param {string} projectDir - Project directory
   * @returns {string} Output folder name (relative, default: '_rmbaid-output')
   */
  async getOutputFolder(projectDir) {
    const { rmbaidDir } = await this.findRmbaidDir(projectDir);
    return this._readOutputFolder(rmbaidDir);
  }

  /**
   * Handle missing custom module sources interactively
   * @param {Map} customModuleSources - Map of custom module ID to info
   * @param {string} rmbaidDir - RMB_AID directory
   ...
   */
  async handleMissingCustomSources(customModuleSources, rmbaidDir, projectRoot, operation, installedModules, skipPrompts = false) {
    ...
    for (const missing of customModulesWithMissingSources) {
      ...
      switch (action) {
        case 'update': {
          ...
          await this.manifest.addCustomModule(rmbaidDir, missing.info);
        ...
        case 'remove': {
          ...
          if (confirmDelete) {
            ...
            if (typedConfirm === 'DELETE') {
              ...
              const modulePath = path.join(rmbaidDir, missing.id);
              if (await fs.pathExists(modulePath)) {
              ...
              await this.manifest.removeModule(rmbaidDir, missing.id);
              await this.manifest.removeCustomModule(rmbaidDir, missing.id);
  ...
  /**
   * Find the rmbaid installation directory in a project
   * Always uses the standard _rmbaid folder name
   * @param {string} projectDir - Project directory
   * @returns {Promise<Object>} { rmbaidDir: string }
   */
  async findRmbaidDir(projectDir) {
    const rmbaidDir = path.join(projectDir, RMB_AID_FOLDER_NAME);
    return { rmbaidDir };
  }

  /**
   * Read the output_folder setting from module config files
   * Checks rmbaid/config.yaml first, then other module configs
   * @param {string} rmbaidDir - RMB_AID installation directory
   * @returns {string} Output folder path or default
   */
  async _readOutputFolder(rmbaidDir) {
    const yaml = require('yaml');

    // Check rmbaid/config.yaml first (most common)
    const bmmConfigPath = path.join(rmbaidDir, 'rmbaid', 'config.yaml');
    ...
    try {
      const entries = await fs.readdir(rmbaidDir, { withFileTypes: true });
      for (const entry of entries) {
        if (!entry.isDirectory() || entry.name === 'rmbaid' || entry.name.startsWith('_')) continue;
        const configPath = path.join(rmbaidDir, entry.name, 'config.yaml');
