...
const {
  ...
  getInstallToRmbaid: getInstallToRmbaidShared,
...
class ManifestGenerator {
  ...
  /** Delegate to shared skill-manifest module */
  getInstallToRmbaid(manifest, filename) {
    return getInstallToRmbaidShared(manifest, filename);
  }
   ...
   * @param {string} rmbaidDir - _rmbaid
   ...
   */
  async generateManifests(rmbaidDir, selectedModules, installedFiles = [], options = {}) {
    // Create _config directory if it doesn't exist
    const cfgDir = path.join(rmbaidDir, '_config');
    await fs.ensureDir(cfgDir);

    // Store modules list (all modules including preserved ones)
    const preservedModules = options.preservedModules || [];

    // Scan the rmbaid directory to find all actually installed modules
    const installedModules = await this.scanInstalledModules(rmbaidDir);
    ...
    this.rmbaidDir = rmbaidDir;
    this.rmbaidFolderName = path.basename(rmbaidDir); // Get the actual folder name (e.g., '_rmbaid' or 'rmbaid')
   ...
   * Manifest YAML is loaded only when present — for install_to_rmbaid and agent metadata.
   * Populates this.skills[] and this.skillClaimedDirs (Set of absolute paths).
   */
  async collectSkills() {
    this.skills = [];
    this.skillClaimedDirs = new Set();
    const debug = process.env.RMB_AID_DEBUG_MANIFEST === 'true';

    for (const moduleName of this.updatedModules) {
      const modulePath = path.join(this.rmbaidDir, moduleName);
      ...
      const walk = async (dir) => {
        ...
        if (skillMeta) {
          // Load manifest when present (for install_to_rmbaid and agent metadata)
          ...
          const installPath = relativePath
            ? `${this.rmbaidFolderName}/${moduleName}/${relativePath}/${skillFile}`
            : `${this.rmbaidFolderName}/${moduleName}/${skillFile}`;
          ...
          if (manifest && manifest.__single && manifest.__single.canonicalId && artifactType !== 'agent') {
            console.warn(
              `Warning: Native entrypoint manifest at ${dir}/rmbaid-skill-manifest.yaml contains canonicalId — this field is ignored for SKILL.md directories (directory name is the canonical ID)`,
            );
          }
          ...
          this.skills.push({
            ...
            path: installPath,
            canonicalId,
            install_to_rmbaid: this.getInstallToRmbaid(manifest, skillFile),
          });
  ...
  async collectAgents(selectedModules) {
    ...
    const standaloneAgentsDir = path.join(this.rmbaidDir, 'agents');
   ...
   * Discovers agents via directory with rmbaid-skill-manifest.yaml containing type: agent
   *
  ...
  async getAgentsFromDirRecursive(dirPath, moduleName, relativePath = '', debug = false) {
    ...
    for (const entry of entries) {
      ...
      if (dirManifest && dirManifest.__single && dirManifest.__single.type === 'agent') {
        ...
        const installPath = `${this.rmbaidFolderName}/${agentModule}/${dirRelativePath}`;
  ...
  async writeMainManifest(cfgDir) {
    ...
    for (const moduleName of this.modules) {
      ...
      const versionInfo = await manifestObj.getModuleVersionInfo(moduleName, this.rmbaidDir);
  ...
  async writeSkillManifest(cfgDir) {
    ...
    let csvContent = 'canonicalId,name,description,module,path,install_to_rmbaid\n';
    ...
    for (const skill of this.skills) {
      const row = [
        ...
        escapeCsv(skill.install_to_rmbaid),
      ].join(',');
      csvContent += row + '\n';
    }
  ...
  async writeFilesManifest(cfgDir) {
    ...
    if (this.allInstalledFiles && this.allInstalledFiles.length > 0) {
      ...
      for (const filePath of this.allInstalledFiles) {
        // Store paths relative to rmbaidDir (no folder prefix)
        const relativePath = filePath.replace(this.rmbaidDir, '').replaceAll('\\', '/').replace(/^\//, '');
      ...
      for (const file of this.files) {
        ...
        const relPath = file.path.replace(this.rmbaidFolderName + '/', '');
        const filePath = path.join(this.rmbaidDir, relPath);
        const hash = await this.calculateFileHash(filePath);
        allFiles.push({
          ...file,
          path: relPath,
  ...
  /**
   * Scan the rmbaid directory to find all installed modules
   * @param {string} rmbaidDir - Path to rmbaid directory
   * @returns {Array} List of module names
   */
  async scanInstalledModules(rmbaidDir) {
    const modules = [];
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        const modulePath = path.join(rmbaidDir, entry.name);
