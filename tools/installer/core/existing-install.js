...
/**
 * Immutable snapshot of an existing RMB_AID installation.
...
class ExistingInstall {
  ...
  /**
   * Scan a rmbaid directory and return an immutable snapshot of what's installed.
   * @param {string} rmbaidDir - Path to rmbaid directory
   * @returns {Promise<ExistingInstall>}
   */
  static async detect(rmbaidDir) {
    ...
    const manifestData = await manifest.read(rmbaidDir);
    if (manifestData) {
    ...
    const corePath = path.join(rmbaidDir, 'core');
    ...
    if (manifestData && manifestData.modules && manifestData.modules.length > 0) {
      for (const moduleId of manifestData.modules) {
        const modulePath = path.join(rmbaidDir, moduleId);
