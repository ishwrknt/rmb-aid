...
- **Severity:** HIGH
- **Applies to:** `SKILL.md`
- **Rule:** The `name` value must start with `rmbaid-`, use only lowercase letters, numbers, and single hyphens between segments.
- **Detection:** Regex test: `^rmbaid-[a-z0-9]+(-[a-z0-9]+)*$`.
- **Fix:** Rename to comply with the format (e.g., `rmbaid-my-skill`).
...
- **Exception:** `rmbaid-agent-tech-writer` — has sub-skill files with intentional `name` fields (to be revisited).
...
- **Exception:** `rmbaid-agent-tech-writer` — has sub-skill files with intentional `description` fields (to be revisited).
...
- **Detection:** For each external file reference (frontmatter values, markdown links, inline paths), check whether the resolved path points into a directory that is or contains a skill (has a `SKILL.md`). Patterns to flag:
  - `{project-root}/_rmbaid/.../other-skill/anything.md`
  - `{project-root}/_rmbaid/.../other-skill/steps/...`
  - `{project-root}/_rmbaid/.../other-skill/templates/...`
...
- **Fix:**
  ...
  - If the intent is to use a shared resource (template, data file): the resource should be extracted to a shared location outside both skills (e.g., `core/data/`, `rmbaid/data/`, or a config-referenced path) — not reached into from across skill boundaries.
...
- **Detection:** Find all references to other skills by name (typically backtick-quoted skill names like \`rmbaid-foo\`). Check the surrounding instruction text (same sentence or directive) for file-oriented verbs: "read", "follow", "load", "execute", "run", "open". Flag any that do not use "invoke" (or a close synonym like "activate" or "launch").
