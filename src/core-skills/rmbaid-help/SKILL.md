---
name: rmbaid-help
description: 'Analyzes current state and user query to answer RMB_AID questions or recommend the next skill(s) to use. Use when user asks for help, rmbaid help, what to do next, or what to start with in RMB_AID.'
---
...
Help the user understand where they are in their RMB_AID workflow and what to do next. Answer RMB_AID questions when asked.
...
- **Catalog**: `{project-root}/_rmbaid/_config/rmbaid-help.csv` — assembled manifest of all installed module skills
- **Config**: `config.yaml` and `user-config.yaml` files in `{project-root}/_rmbaid/` and its subfolders — resolve `output-location` variables, provide `communication_language` and `project_knowledge`
...
- Skill name in backticks — e.g., `rmbaid-create-prd`
