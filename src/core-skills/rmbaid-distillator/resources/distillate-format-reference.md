...
platforms with no shared methodology layer. RMB_AID is uniquely positioned to be the
...
building on the open-source skills CLI ecosystem, RMB_AID offloads the highest-churn
...
```
## Differentiation
- Anti-fragmentation positioning: RMB_AID = cross-platform constant across 40+ fragmenting AI tools; no competitor provides shared methodology layer
- Platform complexity delegated to Vercel skills CLI ecosystem (MIT); RMB_AID maintains methodology, not platform configs
```
...
- **Key gap**: NO competitor combines structured methodology with plugin
  marketplace — this is RMB_AID's whitespace
```
...
**Brief says:**
```
rmbaid-help must always be included as a base skill in every bundle
```

**Discovery notes say:**
```
rmbaid-help must always be included as a base skill in every bundle/install
(solves discoverability problem)
```

**Distillate keeps the more contextual version:**
```
- rmbaid-help: always included as base skill in every bundle (solves discoverability)
```
...
type: rmbaid-distillate
sources:
  - "product-brief-rmbaid-next-gen-installer.md"
  - "product-brief-rmbaid-next-gen-installer-discovery-notes.md"
downstream_consumer: "PRD creation"
created: "2026-03-13"
token_estimate: 1450
parts: 1
...
## Core Concept
- RMB_AID Next-Gen Installer: replaces monolithic Node.js CLI with skill-based plugin architecture for distributing RMB_AID methodology across 40+ AI platforms
- Three layers: self-describing plugins (rmbaid-manifest.json), cross-platform install via Vercel skills CLI (MIT), runtime registration via rmbaid-setup skill
- Transforms RMB_AID from dev-only methodology into open platform for any domain (creative, therapeutic, educational, personal)
...
- Plugins: skill bundles with Anthropic plugin standard as base format + rmbaid-manifest.json extending for RMB_AID-specific metadata (installer options, capabilities, help integration, phase ordering, dependencies)
- Existing manifest example: `{"module-code":"rmbaid","replaces-skill":"rmbaid-create-product-brief","capabilities":[{"name":"create-brief","menu-code":"CB","supports-headless":true,"phase-name":"1-analysis","after":["brainstorming"],"before":["create-prd"],"is-required":true}]}`
- Vercel skills CLI handles platform translation; integration pattern (wrap/fork/call) is PRD decision
- rmbaid-setup: global skill scanning installed rmbaid-manifest.json files, registering capabilities, configuring project settings; always included as base skill in every bundle (solves bootstrapping)
- rmbaid-update: plugin update path without full reinstall; technical approach (diff/replace/preserve customizations) is PRD decision
...
## Differentiation
- Anti-fragmentation: RMB_AID = cross-platform constant; no competitor provides shared methodology layer across AI tools
- Curated quality: all submissions gated, human-reviewed by RMB_AID + core team; 13.4% of community skills have critical vulnerabilities (Snyk 2026); quality gate value increases as ecosystem gets noisier
- Domain-agnostic: no competitor builds beyond software dev workflows; same plugin system powers any domain via RMB_AID Builder (separate initiative)
...
- Future plugin creators: non-dev authors using RMB_AID Builder; need distribution without building own installer
...
- rmbaid-setup discovers/registers all plugins from manifests; clear errors for malformed manifests
- At least one external module author successfully publishes plugin using manifest system
- rmbaid-update works without full reinstall
- Existing CLI users have documented migration path

## Scope
- In: manifest spec, rmbaid-setup, rmbaid-update, Vercel CLI integration, NPX installer, zip bundles, migration path
- Out: RMB_AID Builder, marketplace web platform, skill conversion (prerequisite, separate), one-click install for all platforms, monetization, quality certification process (gated-submission principle is architectural requirement; process defined separately)
...
- Entry: `tools/installer/rmbaid-cli.js` (Commander.js) → `tools/installer/core/installer.js`
...
- Skills already use directory-per-skill layout; rmbaid-manifest.json sidecars exist but are not source of truth
...
- rmbaid-update mechanics: diff/replace? Preserve user customizations?
...
- rmbaid-manifest.json as open standard submission to Agent Skills governance?
...
## Opportunities
- Module authors as acquisition channel: each published plugin distributes RMB_AID to creator's audience
- CI/CD integration: rmbaid-setup as pipeline one-liner increases stickiness
- Educational institutions: structured methodology + non-technical install → university AI curriculum
- Skill composability: mixing RMB_AID modules with third-party skills for custom methodology stacks
