...
Detect if PRD follows RMB_AID format and route appropriately - classify as RMB_AID Standard / RMB_AID Variant / Non-Standard, with optional parity check for non-standard formats.
...
### 2. Check for RMB_AID PRD Core Sections

Check if the PRD contains the following RMB_AID PRD core sections:
...
- Follows RMB_AID PRD structure closely

**RMB_AID Variant:**
- 3-4 core sections present
- Generally follows RMB_AID patterns but may have structural differences
- Missing some sections but recognizable as RMB_AID-style

**Non-Standard:**
- Fewer than 3 core sections present
- Does not follow RMB_AID PRD structure
...
**Format Classification:** [RMB_AID Standard / RMB_AID Variant / Non-Standard]
**Core Sections Present:** [count]/6
```

### 5. Route Based on Format Classification

**IF format is RMB_AID Standard or RMB_AID Variant:**
...
This PRD does not follow RMB_AID standard structure (only {count}/6 core sections present).
...
**[A] Parity Check** - Analyze gaps and estimate effort to reach RMB_AID PRD parity
...
- RMB_AID core sections checked systematically
- Format classified correctly based on section count
- Findings reported to validation report
- RMB_AID Standard/Variant PRDs proceed directly to next validation step
